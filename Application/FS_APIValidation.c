...
static void _FileAPITest(void)  {
  ...
  _Log("Ok\n");
  //
  //  5. Remove opened file
  //
  _Log("Try to remove an opened file...");
  if (FS_Remove("File.txt")) {
    _Log("OK (Not removed)\n");
  } else {
    _ErrorOut("Failed (File removed)\n");
  }
  //
  //  6. Remove closed file
  //
  _Log("Try to remove an closed file...");
  FS_FClose(pFile);
  if (FS_Remove("File.txt")) {
    _ErrorOut("Failed to remove file\n");
  } else {
    _Log("OK (File removed)\n");
  }
  //
  //  7. Create a bunch of files
  //
  ...
  _Log("Ok\n");
  //
  // 8. Remove all files that have been created
  //
  _Log("Removing created files in root directory");
  for (i = 0; i < NUM_FILES; i++) {
    char acFileName[20];
