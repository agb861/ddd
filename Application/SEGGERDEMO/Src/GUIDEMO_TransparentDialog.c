...
static void _cbWin(WM_MESSAGE * pMsg) {
  ...
  case WM_PAINT:
	...
	if (xSizeWin > xSizeBmp) {
      GUI_ClearRect(xSizeBmp, 0, xSizeWin - 1, ySizeWin - 1);
	}
	if (ySizeWin > ySizeBmp) {
      GUI_ClearRect(ySizeBmp, 0, xSizeBmp - 1, ySizeWin - 1);
	}
	*/
    xSizeWin = WM_GetWindowSizeX(hWin);
    ySizeWin = WM_GetWindowSizeY(hWin);
	...
	if (xPos) {
	  GUI_ClearRect(0, 0, xPos - 1, ySizeWin - 1);
	}
	if ((xPos + xSizeBmp) < xSizeWin) {
	  GUI_ClearRect(xPos + xSizeBmp, 0, xSizeWin - 1, ySizeWin - 1);
	}
	if (yPos) {
	  GUI_ClearRect(xPos, 0, xPos + xSizeBmp - 1, yPos - 1);
	}
	if ((yPos + ySizeBmp) < ySizeWin) {
	  GUI_ClearRect(xPos, yPos + ySizeBmp, xPos + xSizeBmp - 1, ySizeWin - 1);
	}
    break;
  }
}
