...
static void _HidTask(void) {
  ...
  while (1) {
    ...
    if ((_EventOccurred & (MOUSE_EVENT)) == MOUSE_EVENT) {
      ...
      x = _PidState.x;
      y = _PidState.y;
      x += _MouseData.xChange;
      y += _MouseData.yChange;
      _PidState.Pressed = _MouseData.ButtonState;
      if ((x >= 0) && (x <= xSize)) {
        _PidState.x = x;
      }
      if ((y >= 0) && (y <= ySize)) {
...
void MainTask(void) {
  GUI_Init();
#if USE_USB_HOST_HID
  OS_CREATETASK(&_TCBHid, "USBH_HIDTask", _HidTask, TASKPRIO_USB,      _StackHid);    // Start USBH Hid task
#endif
#if (INCLUDE_IP == 1)
  IP_Init();
  //
