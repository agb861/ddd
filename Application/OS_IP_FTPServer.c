...
#define TASK_PRIO_FTPS_CHILD     100
#define TASK_PRIO_FTPS_PARENT    101
#define TASK_PRIO_IP_MAIN        110 // Priority should be higher as all TCP/IP application tasks
#define TASK_PRIO_IP_RX          115 // Must be the highest priority of all TCP/IP related tasks
...
// Task stacks
//
static OS_STACKPTR int _Stack[768/sizeof(int)];         // Define the stack of the IPTask to 768 bytes
static OS_STACKPTR int _IPRxStack[512/sizeof(int)];     // Define the stack of the RxTask to 512 bytes
//
//  Task-control-blocks
//
static OS_TASK         _TCB;
  ...
  while (1) {
    ...
    if (_ConnectCnt < MAX_CONNECTIONS) {
      for (i = 0; i < MAX_CONNECTIONS; i++) {
        ...
        if (r == 0) {
          OS_EnterRegion();
          OS_CREATETASK_EX(&_aFTPTasks[i], "IP_FTPServerChild", _FTPServerChildTask, TASK_PRIO_FTPS_CHILD, _aFTPStacks[i], (void *)hSock);
          _AddToConnectCnt(1);
          OS_LeaveRegion();
          break;
        }
...
void MainTask(void) {
  ...
  OS_SetPriority(OS_GetTaskID(), TASK_PRIO_FTPS_PARENT);                             // This task has highest prio!
