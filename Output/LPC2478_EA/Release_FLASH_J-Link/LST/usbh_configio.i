...
#line 25 "Config\\USBH_ConfigIO.c"
#line 1 "USBH\\USBH_Int.h"
...
#line 28 "USBH\\USBH_Int.h"
#line 29 "USBH\\USBH_Int.h"
...
typedef struct USBH_GLOBAL {
  unsigned char                      ConfigCompleted;
  unsigned char                      InitCompleted;
  USBH_HC_HANDLE          hHC;
  USBH_HC_BD_HANDLE       hHCBD;
  USBH_HOST_DRIVER_INST   DriverInst;
  USBH_HOST_DRIVER      * pDriver;
  struct {
    unsigned long TransferBufferSize;
    unsigned char  NumRootHubs;
    unsigned char  RootHubPortsAlwaysPowered;
    unsigned char  RootHubPerPortPowered;
    unsigned char  RootHubSupportOvercurrent;
    unsigned char  NumUSBDevices;
    unsigned char  NumBulkEndpoints;
    unsigned char  NumIntEndpoints;
    unsigned char  NumIsoEndpoints;
    unsigned char  SupportExternalHubs;
  } Config;
} USBH_GLOBAL;
...
#line 155 "USBH\\USBH_Int.h"


#line 163 "USBH\\USBH_Int.h"
...
#line 218 "USBH\\USBH_Int.h"

typedef struct URB_BUFFER_POOL {
  unsigned long                   Magic;
  USBH_DLIST            ListEntry;
  unsigned long                   NumberOfBuffer; 
  unsigned long                   BufferCt;       
  unsigned long                   Size;           
...
URB_BUFFER_POOL * USBH_URB_CreateTransferBufferPool(USBH_INTERFACE_HANDLE IfaceHandle, unsigned char Endpoint, unsigned long SizePerBuffer, unsigned long BufferNumbers, int BusMasterTransferMemoryFlag);
...
unsigned long          USBH_URB_GetPendingCounterBufferPool(URB_BUFFER_POOL * Pool);


void * USBH_URB_BufferAllocateTransferBuffer(unsigned long size);
...
void   UbdSetEnumErrorNotificationProcessDeviceEnum(USBH_HUB_PORT   * pPort, DEV_ENUM_STATE state,      USBH_STATUS status, int hub_flag);   
void   UbdSetEnumErrorNotificationHubPortReset(     USBH_HUB_PORT   * pPort, USBH_HUB_PORTRESET_STATE state, USBH_STATUS status);  



#line 291 "USBH\\USBH_Int.h"
...
typedef struct USBH__PNP_NOTIFICATION { 



  USBH_DLIST                 ListEntry;       
  USBH_PNP_NOTIFICATION UbdNotification; 
...
typedef struct INTERFACE_LIST {
  USBH_DLIST   UsbInterfaceEntryList; 
  unsigned int InterfaceCount;        
} INTERFACE_LIST;
...
#line 389 "USBH\\USBH_Int.h"

unsigned char   USBH_ReadReg8  (unsigned char  * pAddr);
unsigned short  USBH_ReadReg16 (unsigned short * pAddr);
void USBH_WriteReg32(unsigned char  * pAddr, unsigned long Value);
unsigned long  USBH_ReadReg32 (unsigned char  * pAddr);
...
typedef struct USBH_OHCI_INFO_GENERAL_TRANS_DESC {
  USBH_HCM_ITEM_HEADER  ItemHeader;        
  OHD_TD_STATUS         Status;            
  char             CancelPendingFlag; 
  unsigned long                   Size;              
...
typedef struct USBH_OHCI_EP0 {
  ...
  USBH_DLIST                        ListEntry;           
  USBH_DLIST                        UrbList;             
  unsigned short                               UrbCount;            
  USBH_URB                        * pPendingUrb;          
  unsigned short                               TdCounter;           
  unsigned char                                AbortMask;
  USBH_EP0_PHASE                    Ep0Phase;            
  unsigned char                              * pSetup;               
  SETUP_BUFFER                    * pSetupPacket;         
  USBH_OHCI_TRANSFER_BUFFER       * pDataPhaseCopyBuffer; 
...
typedef struct USBH_OHCI_BULK_INT_EP { 
  ...
  unsigned short                               UrbCount;          
  USBH_URB                        * pPendingUrb;       
  USBH_OHCI_TRANSFER_BUFFER       * pCopyBuffer;       
  unsigned short                               UrbTotalTdNumber;  
  unsigned short                               UrbDoneTdNumber;   
  unsigned short                               TdCounter;         
  unsigned short                               UpDownTDCounter;   
  unsigned char                                AbortMask;         

  unsigned char                                CancelPendingFlag; 
  USBH_OHCI_DUMMY_INT_EP          * pDummyIntEp;       
...
typedef struct USBH_OHCI_ISO_EP { 
...
typedef struct USBH_OHCI_ED {
  volatile unsigned long Dword0;
  volatile unsigned long TailP;
  volatile unsigned long HeadP;
  volatile unsigned long NextED;
} USBH_OHCI_ED;

#line 607 "USBH\\USBH_Int.h"
...
typedef struct USBH_OHCI_TRANSFER_DESC {
  volatile unsigned long Dword0;
  volatile unsigned long CBP;
  volatile unsigned long NextTD;
  volatile unsigned long BE;
} USBH_OHCI_TRANSFER_DESC;

#line 638 "USBH\\USBH_Int.h"
...
typedef struct USBH_OHCI_ISO_TRANS_DESC {
  volatile unsigned long Dword0;
  volatile unsigned long Dword1;
  volatile unsigned long NextTD;
  volatile unsigned long BE;
  volatile unsigned short OfsPsw[8];
} USBH_OHCI_ISO_TRANS_DESC;




#line 670 "USBH\\USBH_Int.h"
...
typedef struct USBH_OHCI_HCCA_REG { 
  volatile unsigned long InterruptTable[32];
  volatile unsigned short FrameNumber;
...
#line 731 "USBH\\USBH_Int.h"



#line 752 "USBH\\USBH_Int.h"


#line 763 "USBH\\USBH_Int.h"
...
typedef struct USBH_OHCI_HUB_PORT { 
  unsigned char  Port;                     
  unsigned char  Power;                    
...
struct USBH_OHCI_DEVICE { 



  USBH_HCM_POOL                 TransferBufferPool; 
  ...
  USBH_HCM_POOL                 SetupPacketPool;  
  USBH_HCM_POOL                 GTDPool;          
  USBH_HCM_POOL                 IsoTDPool;        
  USBH_HCM_POOL                 HccaPool;         
  USBH_OHCI_HCCA              * pHcca;            
  volatile USBH_OHCI_HCCA_REG * pOhHcca;      
  
  USBH_DLIST                    ControlEpList; 
  unsigned long                           ControlEpCount;
  USBH_TIMER_HANDLE             ControlEpRemoveTimer;
  USBH_TIMER_HANDLE             ControlEpAbortTimer;
  char                     ControlEpRemoveTimerRunFlag;    
  char                     ControlEpRemoveTimerCancelFlag;
  char                     ControlEpAbortTimerRunFlag;
  char                     ControlEpAbortTimerCancelFlag;
  
  USBH_DLIST                    BulkEpList;
  unsigned long                           BulkEpCount;
  USBH_TIMER_HANDLE             hBulkEpRemoveTimer;
  char                     BulkEpRemoveTimerRunFlag;     
  char                     BulkEpRemoveTimerCancelFlag;
  USBH_TIMER_HANDLE             hBulkEpAbortTimer;
  char                     BulkEpAbortTimerRunFlag;      
  char                     BulkEpAbortTimerCancelFlag;
  
  
  
  USBH_OHCI_DUMMY_INT_EP      * DummyInterruptEpArr[(1 + 2 + 4 + 8 + 16 + 32)];
  unsigned long                           IntEpCount;                  
  USBH_TIMER_HANDLE             hIntEpRemoveTimer;
  char                     IntEpRemoveTimerRunFlag;     
  char                     IntEpRemoveTimerCancelFlag;
  USBH_TIMER_HANDLE             hIntEpAbortTimer;
  char                     IntEpAbortTimerRunFlag;      
  char                     IntEpAbortTimerCancelFlag;
  ...
  unsigned long                           IsoEpCount;
  USBH_TIMER_HANDLE             hIsoEpRemoveTimer;
  char                     IsoEpRemoveTimerRunFlag;     
  char                     IsoEpRemoveTimerCancelFlag;
  USBH_TIMER_HANDLE             hIsoEpAbortTimer;
  char                     IsoEpAbortTimerRunFlag;      
  char                     IsoEpAbortTimerCancelFlag;
  
  USBH_TIMER_HANDLE             hInitDeviceTimer;            
  volatile unsigned short                  UpperFrameCounter;           
  volatile unsigned short                  LastFrameCounter;
  unsigned char                          * pRegBase;      
  unsigned long                           FmIntervalReg; 
  USBH_OHCI_DEV_STATE           State;  
...
USBH_STATUS     USBH_OHCI_EP0_Init                  (USBH_OHCI_EP0            * Ep,  USBH_OHCI_DEVICE * pDev, unsigned long Mask, unsigned char DeviceAddress, unsigned char EndpointAddress, unsigned short MaxFifoSize, USBH_SPEED Speed);
...
void            USBH_OHCI_Ep0RemoveEndpoints        (USBH_OHCI_DEVICE     * pDev, char AllEndpointFlag);
void            USBH_OHCI_EP0_OnReleaseEpTimer      (void               * pContext); 
void            USBH_OHCI_EP0_OnAbortUrbTimer       (void               * pContext);
USBH_STATUS     USBH_OHCI_EP0_Alloc                 (USBH_HCM_POOL * EpPool, USBH_HCM_POOL * SetupPacketPool, unsigned long Numbers);  
void            USBH_OHCI_Ep0Free                   (USBH_HCM_POOL * EpPool); 
...
void                       USBH_OHCI_EpGlobInitED              (USBH_HCM_ITEM_HEADER * Header,   unsigned char                DeviceAddr,  unsigned char    EpWithDirection, unsigned long MaxPktSize, char IsoFlag, char SkipFlag, USBH_SPEED Speed);
void                       USBH_OHCI_EpGlobInsertTD            (USBH_HCM_ITEM_HEADER * EpHeader, USBH_HCM_ITEM_HEADER * NewTdHeader, unsigned short * TdCounter);
void                       USBH_OHCI_EpGlobDeleteAllPendingTD  (USBH_HCM_ITEM_HEADER * EpHeader, unsigned short             * TdCounter);
void                       USBH_OHCI_EpGlobLinkEds             (USBH_HCM_ITEM_HEADER * Last,     USBH_HCM_ITEM_HEADER * New);
unsigned long                        USBH_OHCI_EpGlobUnlinkEd            (USBH_HCM_ITEM_HEADER * Prev,     USBH_HCM_ITEM_HEADER * Remove);
void                       USBH_OHCI_EpGlobRemoveAllTDtoPool   (USBH_HCM_ITEM_HEADER * EpHeader, unsigned short * pTdCounter);
...
void                       USBH_OHCI_EpGlobDeleteDoneTD        (USBH_HCM_ITEM_HEADER * TdItem,   unsigned short * pTdCounter);
...
void                       USBH_OHCI_EpSetToggle               (USBH_HCM_ITEM_HEADER * EpHeader, char Toggle);
char                  USBH_OHCI_EpGetToggle               (USBH_HCM_ITEM_HEADER * EpHeader);
unsigned long                        USBH_OHCI_EpGlobGetTdCount          (USBH_HCM_ITEM_HEADER * EpHeader, USBH_HCM_POOL * TdPool);
...
USBH_OHCI_BULK_INT_EP * USBH_OHCI_BULK_INT_InitEp                    (USBH_OHCI_BULK_INT_EP * Ep, USBH_OHCI_DEVICE * Dev, unsigned char EndpointType, unsigned char DeviceAddress, unsigned char EndpointAddress, unsigned short MaxFifoSize, unsigned short IntervalTime, USBH_SPEED Speed, unsigned long Flags);
...
char               USBH_OHCI_INT_RemoveEDFromLogicalListAndFree (USBH_OHCI_DEVICE * pDev, char All);
unsigned long                     USBH_OHCI_INT_GetBandwidth                   (USBH_OHCI_DEVICE * pDev, unsigned short IntervalTime, USBH_OHCI_DUMMY_INT_EP ** ppEp);

void                    USBH_OHCI_BULK_RemoveEps                     (USBH_OHCI_DEVICE * pDev, char AllEndpointFlag);
...
USBH_OHCI_ISO_EP * USBH_OHCI_ISO_InitEp                   (USBH_OHCI_ISO_EP * Ep, USBH_OHCI_DEVICE * Dev, unsigned char EndpointType, unsigned char DeviceAddress, unsigned char EndpointAddress, unsigned short MaxFifoSize, unsigned short IntervalTime, USBH_SPEED Speed, unsigned long Flags);
...
void               USBH_OHCI_ISO_RemoveEps                (USBH_OHCI_DEVICE * dev, char AllEndpointFlag);

USBH_STATUS                      USBH_OHCI_TdGetStatusAndLength(USBH_OHCI_INFO_GENERAL_TRANS_DESC  * Gtd,            unsigned long * Transferred,  char     * shortPkt);
USBH_STATUS                      USBH_OHCI_TdAlloc             (USBH_HCM_POOL * GeneralTd,      unsigned long   GeneralTdNumbers, unsigned Alignment);
void                             USBH_OHCI_TdInit              (USBH_OHCI_INFO_GENERAL_TRANS_DESC  * gtd, void * Ep, unsigned char    EndpointType, USBH_OHCI_TD_PID   Pid, unsigned long StartAddr, unsigned long EndAddr, unsigned long Dword0Mask);
USBH_OHCI_INFO_GENERAL_TRANS_DESC * USBH_OHCI_GetTransDesc     (USBH_HCM_POOL * GeneralTd);
unsigned long                              USBH_OHCI_TdGetNextTd         (unsigned long             TdAddress);
void                             USBH_OHCI_IsoTdInit           (USBH_OHCI_INFO_GENERAL_TRANS_DESC * gtd, USBH_OHCI_ISO_EP * pIsoEp, USBH_ISO_REQUEST * pIsoRequest, unsigned long DWord0, unsigned long StartAddr, int NumBytes, int Index);
USBH_STATUS                      USBH_OHCI_ISO_TdGetStatusAndLength(USBH_OHCI_INFO_GENERAL_TRANS_DESC * pGlobalTransDesc, unsigned long * pTransferred, char * pShortPacket);
...
USBH_HC_EP_HANDLE    USBH_OHCI_AddEndpoint     (USBH_HC_HANDLE    hHostController, unsigned char EndpointType, unsigned char DeviceAddress, unsigned char EndpointAddress, unsigned short MaxFifoSize, unsigned short IntervalTime, USBH_SPEED Speed);

unsigned long                  USBH_OHCI_GetFrameNumber         (USBH_HC_HANDLE     hHostController);
USBH_OHCI_TRANSFER_BUFFER * USBH_OHCI_GetCopyTransferBuffer           (USBH_HCM_POOL           * pTransferBufferPool);
USBH_OHCI_TRANSFER_BUFFER * USBH_OHCI_GetInitializedCopyTransferBuffer(USBH_HCM_POOL           * pTransferBufferPool, unsigned char * pUrbBuffer, unsigned long UrbBufferLength);
unsigned long                  USBH_OHCI_FillCopyTransferBuffer                 (USBH_OHCI_TRANSFER_BUFFER * pTransferBuffer);
unsigned long                  USBH_OHCI_CopyToUrbBufferUpdateTransferBuffer    (USBH_OHCI_TRANSFER_BUFFER * pTransferBuffer,     unsigned long   Transferred);
unsigned long                  USBH_OHCI_UpdateCopyTransferBuffer               (USBH_OHCI_TRANSFER_BUFFER * pTransferBuffer,     unsigned long   Transferred);
unsigned char                 * USBH_OHCI_GetBufferLengthFromCopyTransferBuffer  (USBH_OHCI_TRANSFER_BUFFER * pTransferBuffer,     unsigned long * pLength);
void                 USBH_OHCI_UpdateUpperFrameCounter                (USBH_OHCI_DEVICE * pDev);
void                 USBH_OHCI_EndpointListEnable                     (USBH_OHCI_DEVICE * pDev, unsigned char EpType, char Enable, char ListFill);
...
unsigned long          USBH_OHCI_ROOTHUB_GetHubStatus    (USBH_HC_HANDLE hHostController);
void         USBH_OHCI_ROOTHUB_ClearHubStatus  (USBH_HC_HANDLE hHostController,          unsigned short FeatureSelector);
void         USBH_OHCI_ROOTHUB_ClearPortStatus (USBH_HC_HANDLE hHostController, unsigned char Port, unsigned short FeatureSelector);
unsigned long          USBH_OHCI_ROOTHUB_GetPortStatus   (USBH_HC_HANDLE hHostController, unsigned char Port);
void         USBH_OHCI_ROOTHUB_SetPortPower    (USBH_HC_HANDLE hHostController, unsigned char Port, unsigned char PowerOn);
void         USBH_OHCI_ROOTHUB_ResetPort       (USBH_HC_HANDLE hHostController, unsigned char Port);
void         USBH_OHCI_ROOTHUB_DisablePort     (USBH_HC_HANDLE hHostController, unsigned char Port);
void         USBH_OHCI_ROOTHUB_SetPortSuspend  (USBH_HC_HANDLE hHostController, unsigned char Port, USBH_PORT_POWER_STATE State);
...
void           * USBH_MallocZeroed                   (unsigned long                  Size);
void           * USBH_TryMalloc                      (unsigned long                  Size);
void           * USBH_AllocTransferMemory            (unsigned long                  NumBytes, unsigned Alignment);
...
signed long          USBH_BringInBounds(signed long v, signed long Min, signed long Max);
unsigned long          USBH_LoadU32BE(const unsigned char * pData);
unsigned long          USBH_LoadU32LE(const unsigned char * pData);
unsigned long          USBH_LoadU32TE(const unsigned char * pData);
unsigned     USBH_LoadU16BE(const unsigned char * pData);
unsigned     USBH_LoadU16LE(const unsigned char * pData);
void         USBH_StoreU16BE     (unsigned char * p, unsigned v);
void         USBH_StoreU16LE     (unsigned char * p, unsigned v);
void         USBH_StoreU32BE     (unsigned char * p, unsigned long      v);
void         USBH_StoreU32LE     (unsigned char * p, unsigned long      v);
unsigned long          USBH_SwapU32                (unsigned long      v);
...
void USBH__ConvSetupPacketToBuffer(const USBH_SETUP_PACKET * Setup, unsigned char * pBuffer);
...
#line 26 "Config\\USBH_ConfigIO.c"
#line 1 "OS\\RTOS.h"
...
#line 27 "Config\\USBH_ConfigIO.c"
